...
## Using the Sui client {#using-the-sui-client}
...
### Paying For transactions with gas objects {#paying}
...
## Create new account addresses {#create-account-addresses}
...
### Create a new account address {#new-account-address}
...
### Add existing accounts to client.yaml {#accounts-clientyaml}
...
## View objects an address owns {#view-objects-an-address-owns}
...
## Merge and split coin objects {#merge-and-split}
...
## Calling Move code {#calling-move-code}
...
-   `--args` - a list of function arguments formatted as
    [SuiJSON](../../../reference/json-rpc/json-rpc-format.mdx) values (hence the preceding `0x` in address
    and object ID):
...
## Customize genesis {#customize-genesis}
