...
1.  Set up your fork of the Sui repository:
    ...
    2.  Clone your personal fork of the Sui repository to your local machine
...
2.  Make a copy of the [Full node YAML template](https://github.com/MystenLabs/sui/blob/main/crates/sui-config/data/fullnode-template.yaml):
    ```bash
...
3.  Download the genesis blob for the network to use:
    ...
    -   [Mainnet genesis blob](https://github.com/MystenLabs/sui-genesis/raw/main/mainnet/genesis.blob)
        ...
        ```
4.  Testnet Full nodes only: Edit the `fullnode.yaml` file to include peer nodes for state synchronization. Append the following to the end of the current configuration:
    ```bash
    p2p-config:
      seed-peers:
...
2.  Start the Sui Full node:
    ...
    ```
3.  Optional: [Publish/subscribe](../../learn/core-concepts/event-query-and-subscription.mdx#subscribe-to-sui-events) to notifications using JSON-RPC via websocket.
