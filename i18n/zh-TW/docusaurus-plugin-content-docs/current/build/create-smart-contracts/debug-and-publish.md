...
At this point, however, the `sui move` command does not support package publishing. In fact, it is not clear if it even makes sense to accommodate package publishing, which happens once per package creation, in the context of a unit testing framework. Instead, you can use the Sui CLI client to [publish](../setup/cli/client-cli.md#publish-packages) Move code and to [call](../setup/cli/client-cli.md#calling-move-code) that code. See the [Sui CLI client documentation](../setup/cli/client-cli.md) for information on how to publish the package created in this tutorial.
