...
-   Four validator nodes operated by Mysten Labs. Clients send transactions and read requests via this endpoint: `https://fullnode.<SUI-NETWORK-VERSION>.sui.io:443` using [JSON-RPC](../../reference/json-rpc/read-api.mdx).
-   A public network [Sui Explorer](https://suiexplorer.com/) for browsing transaction history.

You can [request test SUI tokens](#request-test-tokens) through the Sui [devnet-faucet](https://discordapp.com/channels/916379725201563759/971488439931392130) and [testnet-faucet](https://discord.com/channels/916379725201563759/1037811694564560966) Discord channels, depending on which version of the network you use. If connected to Localnet, use cURL to request tokens from your [local faucet](local-network.mdx#use-local-faucet). The coins on these networks have no financial value. For obvious reasons, there is no faucet service for Mainnet.
...
## Configure Sui client {#configure-sui-client}
...
### Connect to a custom RPC endpoint {#custom-rpc-endpoint}
...
2.  Get your Sui client address: `sui client active-address`
3.  Request test SUI tokens in the Sui [#devnet-faucet](https://discord.com/channels/916379725201563759/971488439931392130) or [#testnet-faucet](https://discord.com/channels/916379725201563759/1037811694564560966) Discord channel. Send the following message to the relevant channel with your client address: `!faucet <YOUR-CLIENT-ADDRESS>`. If you have a local network, programmatically request tokens from your [local faucet](local-network.mdx#use-local-faucet).

## Publish a Move module {#publish-move-module}
...
## Make a Move call {#make-a-move-call}
