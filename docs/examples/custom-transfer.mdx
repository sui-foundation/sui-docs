...
slug: /custom-transfer
---

Objects defined with only the `key` ability in Sui Move cannot be transferred by default. To enable transfers, the publisher has to create a custom transfer function. This function can include any arguments, for example a fee, that users have to pay to transfer.
