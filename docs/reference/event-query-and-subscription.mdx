...
Sui [full nodes](../contribute/nodes/full-node.mdx) support publish / subscribe using [JSON-RPC](../reference/json-rpc/read-api.mdx) notifications via the WebSocket API. You can use this service with Sui client to filter and subscribe to a real-time event stream generated from Move or from the Sui network.
